...
function findBurgForMFCG (params) {
  ...
  for (let p of referrer.searchParams) {
    if (p[0] === 'name') b.name = p[1] else if (p[0] === 'size') b.population = +p[1] else if (p[0] === 'seed') b.MFCG = +p[1] else if (p[0] === 'shantytown') b.shanty = +p[1] else
...
function applyDefaultBiomesSystem () {
  ...
  // 应该是图标数据
  const icons = [{}, { dune: 3, cactus: 6, deadTree: 1 }, { dune: 9, deadTree: 1 }, {
    acacia: 1,
    grass: 9
  }, { grass: 1 }, { acacia: 8, palm: 1 }, { deciduous: 1 }, {
    acacia: 5,
    palm: 3,
    deciduous: 1,
    swamp: 1
...
function drawCoastline () {
  ...
  function connectVertices (start, t) {
    ...
    for (let i = 0, current = start; i === 0 || current !== start && i < 50000; i++) {
      ...
      if (v[0] !== prev && c0 !== c1) current = v[0] else if (v[1] !== prev && c1 !== c2) current = v[1] else if (v[2] !== prev && c0 !== c2) current = v[2]
...
function addZones (number = 1) {
  ...
  function addFlood () {
    const fl = cells.fl.filter(fl => fl), meanFlux = d3.mean(fl), maxFlux = d3.max(fl),
